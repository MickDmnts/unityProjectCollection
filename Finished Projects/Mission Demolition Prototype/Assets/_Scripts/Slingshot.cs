...
public class Slingshot : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        projectile.transform.position = launchPos; //The position of the projectile in the middle of the slingshot
        projectileRigidbody = projectile.GetComponent<Rigidbody>(); //gets the projectiles rb
        projectileRigidbody.isKinematic = true; //Set the rigidbody to kinematic, so it wont move on its own
    }
    ...
    private void Update()
    {
        ...
        launchPoint.SetActive(false); //deactivate the halo ring while holding down mouse

        Vector3 mouseDelta = mousePos3D - launchPos;
        float maxLength = GetComponent<SphereCollider>().radius;
